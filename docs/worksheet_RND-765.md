...
## [2025-12-21 ~20:30] Тест MSE_POWER=1.0

### Изменения
- `PYTHON_MSE_POWER`: 0.001 → 1.0
- `DIP_ANGLE_RANGE_DEGREE`: 2 → 5
...
При MSE_POWER=0.001:
- mse^0.001 ≈ 1.0 для любого mse → фактически отключён

При MSE_POWER=1.0:
- mse напрямую влияет на objective

### Результаты
| Метрика | MSE_POWER=0.001 | MSE_POWER=1.0 |
|---------|-----------------|---------------|
| RMSE | 49м | 8.9м |
| Correlation | 0.60 | 0.998 |
| Shift diff | -67.6м vs -19.3м | ближе к reference |

### Проблема
Интерпретация "колбасит тудасюда" — сильные осцилляции между итерациями.
Оптимизатор находит более точное решение, но оно нестабильно.

### Возможные решения
1. Увеличить MSE_POWER до 2.0 (ещё сильнее)
2. Добавить smoothness constraint (штраф за резкие изменения)
3. Включить self-correlation (USE_SELF_CORRELATION_BATCH=True)
4. Уменьшить шаг MD_STEP_METERS (более плавная эволюция)
